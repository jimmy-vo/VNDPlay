...
void USART_OpenPort (USART * serial, uint32_t BaudRate, uint16_t STOPBIT, uint16_t PARITY)
	{
...
void USART1_IRQHandler(void)
{
	if( USART_GetITStatus(USART1, USART_IT_RXNE) )
		{ 																																			  	
			*(USART_1.RX.Buffer + USART_1.RX.BuffPos++) = USART_ReceiveData(USART1);		
			if (USART_1.RX.BuffPos >= USART_1.RX.BuffSize)
			{				
				USART_1.RX.BuffSize = 0;
	...
	if(USART_GetITStatus(USART1, USART_IT_TXE) != RESET)
		{																																																															
			USART_SendData(USART1, *(USART_1.TX.Buffer + USART_1.TX.BuffPos++));		
			if (USART_1.TX.BuffPos >= USART_1.TX.BuffSize)
			{				
				USART_1.TX.BuffSize = 0;
